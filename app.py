...
SPOTIPY_CLIENT_ID='d110343bd92f4a8492dc53e86fe954e0'
SPOTIPY_CLIENT_SECRET='3566581b5d80449286a6c1f4105e0eb6'

auth_manager = SpotifyClientCredentials(client_id=SPOTIPY_CLIENT_ID, client_secret=SPOTIPY_CLIENT_SECRET)
sp = spotipy.Spotify(auth_manager=auth_manager)

st.header('CocoTeen App')
        ...
        if selected_artist_choice == 'Albums':
            ...
            for track in top_songs_result['tracks']:
                with st.container():
                    ...
                    with col3:
                        def feature_requested(track_id=track['id']):
                            track_features = sp.audio_features(track_id)
                            df = pd.DataFrame(track_features, index=[0])
                            df_features = df.loc[:, ['acousticness', 'danceability', 'energy', 'instrumentalness', 'liveness', 'speechiness', 'valence']]
                            with col21:
                        ...
                        feature_button_state = st.button(f'Track Audio Features - {track["id"]}', on_click=feature_requested)

                    with col4:
                        def similar_songs_requested(track_id=track['id']):
                            token = songrecommendations.get_token(SPOTIPY_CLIENT_ID, SPOTIPY_CLIENT_SECRET)
                            similar_songs_json = songrecommendations.get_track_recommendations(track_id, token)
                            recommendation_list = similar_songs_json['tracks']
                            recommendation_list_df = pd.DataFrame(recommendation_list)
                        ...
                        similar_songs_state = st.button(f'Similar Songs - {track["id"]}', on_click=similar_songs_requested)
                    st.write('----')